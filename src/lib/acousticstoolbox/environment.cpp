...
void bellhop::Environment::output_env(std::ostream* os, 
				      std::ostream* ssp, 
				      std::ostream* bty, 
				      std::ostream* trc, 
				      std::ostream* brc, 
...
{
    const double BOX_BUFFER_PERCENT  = 10; // how much to grow the calculation box from the actual sources
    ...
    *os << "'";
    ...
    if(env.surface().medium().type() == protobuf::Environment::Medium::HALF_SPACE)
        *os << env.surface().medium().depth() << " " << env.surface().medium().cp() << " " << env.surface().medium().cs() << " " << env.surface().medium().density() << " " << env.surface().medium().attenuation().value() << " /" << std::endl;
    else if (env.surface().medium().type() ==
	     protobuf::Environment::Medium::REFLECTION_COEFFICIENT)
      {
	...
	for (int j = 0; j < m_coef; j++)
	  eof_coef[j] = env.water_column(k_bathy_max).eof_coef(j);
	for(int i = 0, n = env.water_column(k_bathy_max).sample_size(); i < n; ++i)
        ...
        *os << "0 0 " << std::fixed << std::setprecision(3) << max_depth << std::endl;
        ...
        for(std::map<double, double>::const_iterator it = depth_ssp_map.begin(), end =  depth_ssp_map.end();
            it != end; ++it)
        {
            *os << std::fixed << std::setprecision(3) << it->first << " " << it->second << " /" << std::endl;
            double z = it->first;
    ...
    {    
	    ...
	    eof_coef.resize(m_coef);
	    for (int j = 0; j < m_coef; j++)
	      eof_coef[j] = env.water_column(k).eof_coef(j);
    ...
    switch(env.bottom().medium().type())
    {
        ...
        case protobuf::Environment::Medium::RIGID: *os << 'R'; break;
        case protobuf::Environment::Medium::HALF_SPACE: *os << 'A'; break;
        case protobuf::Environment::Medium::REFLECTION_COEFFICIENT:
	  *os << 'F'; break;
        default: break;
    }
    ...
    *os << "' 0"  << std::endl;
    ...
    if(env.bottom().medium().type() == protobuf::Environment::Medium::HALF_SPACE)
        *os << env.bottom().medium().depth() << " " << env.bottom().medium().cp() << " " << env.bottom().medium().cs() << " " << env.bottom().medium().density() << " " << env.bottom().medium().attenuation().value() << " /" << std::endl;
    else if (env.bottom().medium().type() ==
	     protobuf::Environment::Medium::REFLECTION_COEFFICIENT)
      {
    ...
    *os << "0 " << (int)(max_depth*(1+BOX_BUFFER_PERCENT/100)) << " " << (env.receivers().last().range() / 1000)*(1+BOX_BUFFER_PERCENT/100) << std::endl;
	...
	*bty << env.water_column_size() << std::endl;
	...
	for (int k=0, m = env.water_column_size(); k<m; k++)
	    ...
	    *bty << env.water_column(k).range()/1000 << " "
		 << std::fixed << std::setprecision(3) << env.water_column(k).bathymetry() << std::endl;
        } 
	*ssp << std::endl;

	for(int i = 0, n = rd_ssp_mat[0].size(); i < n; ++i)
        {
	    for (int k=0, m = rd_ssp_mat.size(); k<m; k++)
                *ssp << rd_ssp_mat[k][i] << " "; 
	    *ssp << std::endl;
        }
    }
